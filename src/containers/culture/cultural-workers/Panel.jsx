...
const Panel = () => {
   ...
   useEffect(() => {
      dispatch(getData('cultureWorkers'))
      ...
      <Container>
         ...
         {!isLoading && (
            <Flex width="100%" direction="column" gap="20px">
               <div>
                  <SectionTitle>Маданий кызматкерлер</SectionTitle>
                  {(!!datas.length &&
                     datas.map((item) => (
                        <WorkersList key={item.id} item={item} />
                     ))) || (
                     <Alert severity="info">
                        Маданий кызматкер боюнча маалымат табылган жок:(
                     </Alert>
                  )}
               </div>
            </Flex>
