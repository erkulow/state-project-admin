...
import InitiativiesList from './InitiativiesList'
...
const Panel = () => {
   ...
   useEffect(() => {
      dispatch(getData('youthAffairsYI'))
      ...
      <Container>
         ...
         {!isLoading && (
            <Flex width="100%" direction="column" gap="20px">
               <div>
                  <SectionTitle>Инициативалар</SectionTitle>
                  {(!!datas.length &&
                     datas.map((item) => (
                        <InitiativiesList key={item.id} item={item} />
                     ))) || (
                     <Alert severity="info">
                        Инициативалар боюнча маалымат табылган жок:(
                     </Alert>
                  )}
               </div>
            </Flex>
