...
import HeadOfCommitteeList from './HeadOfCommitteeList'

const Panel = () => {
   const dispatch = useDispatch()
   const { datas } = useSelector((state) => state.crud)
   useEffect(() => {
      dispatch(getData('youthAffairsHC'))
   ...
   return (
      <Container>
         <Flex width="100%" direction="column" gap="20px">
            <div>
               <SectionTitle>Жаштар комитеттери</SectionTitle>
               {(!!datas.length &&
                  datas.map((item) => (
                     <HeadOfCommitteeList key={item.id} item={item} />
                  ))) || (
                  <Alert severity="info">
                     Жаштар комитеттери боюнча маалымат табылган жок:(
                  </Alert>
               )}
            </div>
         </Flex>
