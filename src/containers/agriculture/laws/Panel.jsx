      ...
      <Container>
         ...
         {!isLoading && (
            <Flex width="100%" direction="column" gap="20px">
               <div>
                  <SectionTitle>Айыл-чарба мыйзамдары</SectionTitle>
                  {(!!datas.length &&
                     datas.map((item) => (
                        <LawsList key={item.id} item={item} />
                     ))) || (
                     <Alert severity="info">
                        Айыл-чарба мыйзамдары боюнча маалымат табылган жок:(
                     </Alert>
                  )}
               </div>
            </Flex>
